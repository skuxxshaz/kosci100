...
// Short display titles — the detailed zones/paces/exercises stay in
// session.label, which is now shown as a tap/hover tooltip instead of
// inline text.
export const TYPE_TITLES: Record<SessionType, string> = {
  run: "Run",
  long: "Long Run",
  bike: "Ride",
  stair: "Stair Master",
...
// Short title + a single headline metric (km preferred, else duration).
// Everything else about the session (zones, pace, effort, exercises)
// stays in session.label for the tooltip.
export function sessionTitle(session: PlanSession): { title: string; metric: string | null } {
  const title = TYPE_TITLES[session.type];
  if (session.targetKm) return { title, metric: formatKm(session.targetKm) };
  if (session.targetMin) return { title, metric: formatMin(session.targetMin) };
  return { title, metric: null };
}

// Sum of targetKm across running-type sessions in a week, for a weekly
// running-km target shown in the week header.
const RUN_KM_TYPES: SessionType[] = ["run", "long", "race"];
export function weeklyRunKmTarget(week: PlanWeek): number {
  return Math.round(
    week.sessions.reduce((s, sess) => s + (RUN_KM_TYPES.includes(sess.type) && sess.targetKm ? sess.targetKm : 0), 0)
  );
}
    ...
    weeks: [
      ...
      },
      {
        ...
        sessions: [
          ...
          { date: "2026-09-02", type: "run", label: "Run easy 30min (~5.5km) @ 5:30–5:45/km recovery pace (HR ≤150bpm)", targetMin: 30, targetKm: 5.5 },
          { date: "2026-09-02", type: "gymL", label: "Gym – Lower, light/maintenance: glute bridge, single-leg calf raise, isometric hip hold — last home-based session before Brisbane, keep dose low" },
          { date: "2026-09-03", type: "bike", label: "Rouvy Zone2 45–60min, easy — moved up from Sat since no bike access in Brisbane", targetMin: 60 },
          { date: "2026-09-04", type: "run", label: "Run easy 35min + 4x20sec relaxed strides (build to ~85% effort, full recovery between) — first taste of faster turnover, no bike/gym needed", targetMin: 35, targetKm: 6.5 },
          { date: "2026-09-05", type: "other", label: "Brisbane, no bike access — easy shakeout walk/jog or full rest, whatever the legs want" },
          { date: "2026-09-06", type: "run", label: "Run moderate 50–60min (~10km) @ 5:15–5:25/km (HR ≤155bpm) — first real quality-adjacent session back", targetMin: 55, targetKm: 10 },
        ],
      },
    ],
  },
    ...
    weeks: [
      {
        ...
        sessions: [
          ...
          { date: "2026-10-24", type: "race", label: "Bondi to Manly — race day, 20km. Treat as a quality long run: race effort but don't blow up the back-to-back tomorrow.", targetKm: 20 },
